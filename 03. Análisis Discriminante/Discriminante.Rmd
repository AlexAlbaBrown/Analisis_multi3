...
A diferencia del ADL, el cuadrático no asume la igualdad de matrices de covarianzas. No obstante ello, el problem se mantiene igual,
...
Definimos la **función discriminante cuadrática** aplicando logarítmos y utilizando los estimadores MV para $\mathbf{\mu}_g$ y $\mathbf{\Sigma}_g$ en la función objetivo. De esta forma, tenemos que:
$$\begin{array}{rcl}
...
La $Q_{ig}$ se utiliza para calcular el **score**de la observación $i$ en el grupo $g$, para cada grupo $g = 1, \, \ldots, k$. Así entonces, $i$ se asigna al grupo $g$, sí, y solo si, $Q_{ig} > Q_{ig'} \,\,\, \forall g \neq g'$.
...
\end{array}$$
$$\Pr(Y_i = 1 | \mathbf{x} = \mathbf{x}_i) = \pi_i = \frac{ e^{\mathbf{x}_i' \beta} }{ 1 + e^{\mathbf{x}_i' \beta} }$$
...
Al cociente $\frac{\pi_i }{ 1 - \pi_i } = e^{\mathbf{x}_i' \beta}$ se lo denomina *odds*, y se lo puede conciderar como una medida de riesgo. Es el cociente entre la probabilidad de éxito $\pi_i$ y la probabilidad de fracaso $1 - \pi_i$.
...
y su log-verosimiilitud será:
$$\begin{array}{rcl}
\ell(\beta) & = & \sum\limits_{i = 1}^{n} Y_i \log\left( \frac{ \pi_i }{ 1 - \pi_i } \right) + \sum\limits_{i = 1}^{n} \log \big( 1 - Y_i \big) \\ \\
   & = & \sum\limits_{i = 1}^{n} Y_i \mathbf{x}_i' \beta + \sum\limits_{i = 1}^{n} \log \big( 1 - Y_i \big)
\end{array}$$
...
$$\hat{\pi}_i = \frac{ e^{\mathbf{x}_i' \hat{\mathbf{\beta}}} }{ 1 + e^{\mathbf{x}_i' \hat{\mathbf{\beta}}} }$$
...
Una vez estimado el modelo y calculadas las predicciones $\hat{\pi}_i$, se debe decidir cuales clasificar como éxito ($Y_i = 1$) y cuales como fracaso ($Y_i = 0$). Salvo casos en que otra información esté disponible, lo usual es tomar 0,5 como punto de corte. De esta forma entonces:
$$\hat{Y}_i = \left\{ 
\begin{array}{rcl}
...
$$\text{sensibilidad} = \frac{ \#(\text{éxitos observadors clasificados como éxitos}) }{ \#(\text{éxitos observados}) }$$

$$\text{especificidad} = \frac{ \#(\text{fracasos observadors clasificados como fracasos}) }{ \#(\text{fracasos observados}) }$$
...
El ajuste se basa en que la influencia de las variables explicativas en la variable de respuesta es independienete del punto de corto del logit acumulado. El modelo tiene la siguiente especificación:
$$logit(\theta_{ik}) = \alpha_k + \mathbf{x}_i' \beta$$
donde $i$ indica la observación y $k$ la sub-población o grupo. Los modelos tienen entonces la siguiente forma:
$$\begin{array}{ccccc}
logit(\theta_{i1}) & = & \log \left( \frac{ \pi_{i1} }{ \pi_{i2} + \pi_{i3} + \ldots + \pi_{ik} } \right) & = & \mathbf{x}_i' \beta \\ \\
logit(\theta_{i2}) & = & \log \left( \frac{ \pi_{i1} + \pi_{i2} }{ \pi_{i3} + \pi_{i4} + \ldots + \pi_{ik} } \right) & = & \mathbf{x}_i' \beta \\ \\
\vdots & & \vdots & & \vdots \\ \\
logit(\theta_{ik-1}) & = & \log \left( \frac{ \pi_{i1} + \pi_{i2} + \ldots + \pi_{ik-1} }{ \pi_{ik} } \right) & = & \mathbf{x}_i' \beta \\ \\
\end{array}$$
...
$$logit_{ig} = \log \left( \frac{ \pi_{ig} }{ \pi_{iK} } \right) = \mathbf{x}_{ig}' \beta_g$$
donde $\pi_{ig}$ es la probabilidad de que la observación $i$ pertenezca al grupo $g = 1, \, \ldots, \, K$.

#### Logits de categorías adyacentes

También existen $K - 1$ modelos, pero cada modelo compara su categoría, con la categoría siguiente.
$$logit_{i, \, g} = \log \left( \frac{ \pi_{i, \, g-1} }{ \pi_{i, \, g} } \right) = \mathbf{x}_{i, \, g - 1}' \beta_{g-1}$$
