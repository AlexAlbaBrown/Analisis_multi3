...
\begin{itemize}
\item **Single Linkage**: $d_{(i,j), k} = \min\{ d_{i,k}; \, d_{j,k} \}$
\item **Complete Linkage**: $d_{(i,j), k} = \max\{ d_{i,k}; \, d_{j,k} \}$
\item **Average Linkage**: la distancia entre dos grupos es el prmedio entras las distancias entre pares de observaciones. Es sesgado hacia la formación de grupos con igual varianza.
\item **Centroide**: define la distancia entre los grupos $K$ y $L$ como $d_{(K, L)} = || \bar{x}_K - \bar{x}_L ||^2$. Tiene el beneficio de ser robusto a la presencia de outliers.
\item **Ward**: la unión entre grupos se relaiza de forma tal de minimizar la varianza interna.
...
- Definir un criterio de optimalidad y comprobar si reasignar uno a uno cada elemento de un grupo a otro mejor el criterio.  
...
Este mismo algoritmo puede utilizarse tomando otro tipos de centros. Por ejemplo, k-medoides toma como centros iniciales a los individuos representativos de cada grupo. Por representativos nos referimos a los infividos que minimizan la disimiliaridad promedio a todos los objetos del cluster.
