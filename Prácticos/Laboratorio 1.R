...
M=matrix(data=c(4,3,-2,1,2,-1), nrow=2, ncol=3, byrow=TRUE)
Mt=t(M)
A=M%*%Mt
B=Mt%*%M
...
for(i in 1:dim(miejemplo)[1]){row.names(miejemplo)[i] <- paste0("obs", i)}
...
data <- read.table("miejemplo.txt", header=TRUE ); data <- data[,1:5]
class(data)
names(data)
row.names(data)
dim(data)
...
for(i in 1:dim(data)[1]){row.names(data)[i] <- paste0(i)}
...
for(i in 1:length(a)){print(paste(a[i], b[i]))}
...
X11(15,15)
par(oma=c(0,1,0,1))
plot(seq(-7,7,0.0001),dnorm(seq(-7,7,0.0001),sd=2),col='red', pch=".", ylab=NA, xlab=NA, main="Densidad normal")
...
X11(15,15)
par(oma=c(0,1,0,1))
plot(seq(-7,7,0.0001),pnorm(seq(-7,7,0.0001),sd=2),col='red', pch=".", ylab=NA, xlab=NA, main="Densidad acumulada")
      abline(v=0, col="grey", lty=1)
      abline(h=0, col="grey", lty=1)
...
X11(15,15)
par(oma=c(0,1,0,1))
plot(seq(0,15,0.0001),dchisq(seq(0,15,0.0001),df=3),col='red', pch=".", ylab=NA, xlab=NA, main="Densidad Chi-cuadrado")
      abline(h=0, col="grey", lty=1)


X11(15,15)
par(oma=c(0,1,0,1))
plot(seq(0,15,0.0001),pchisq(seq(0,15,0.0001),df=3),col='red', pch=".", ylab=NA, xlab=NA, main="Densidad acumulada")
      abline(v=0, col="grey", lty=1)
      abline(h=0, col="grey", lty=1)
...
X11(15,15)
par(oma=c(0,1,0,1))
plot(Forbes2000$sales, Forbes2000$profits, col=rgb(0,0,1,0.1), pch=16, ylab="Profits", xlab="Sales", main="Forbes2000")
      abline(h=0, col="grey")
      abline(v=0, col="grey")
